...
from app.schemas.roster_import import SidearmBatchImportSummary, SidearmBatchSchoolResult
...
def test_build_batch_cli_summary_omits_per_school_success_details() -> None:
    summary = SidearmBatchImportSummary(schools_seen=3, schools_selected=2)
    summary.add_result(
        SidearmBatchSchoolResult(
            school_id="S66",
            school_name="Georgetown University",
            success=True,
            summary={"rows_seen": 3, "rows_imported": 2, "players_created": 2},
        )
    )
    summary.add_result(
        SidearmBatchSchoolResult(
    ...
    assert output["rows_seen"] == 3
    assert output["rows_imported"] == 2
    assert output["players_created"] == 2
...
def test_main_all_schools_prints_summary_and_progress(
    ...
    summary = SidearmBatchImportSummary(schools_seen=2, schools_selected=2)
    summary.add_result(
        SidearmBatchSchoolResult(
            school_id="S66",
            school_name="Georgetown University",
    ...
    exit_code = main(["--all-schools", "--season", "2026"])
...
def test_build_parser_supports_all_schools_without_school_id() -> None:
    args = build_parser().parse_args(["--all-schools", "--season", "2026"])

    assert args.all_schools is True
